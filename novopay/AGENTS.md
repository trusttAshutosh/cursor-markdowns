...
- Never comment on Jira or make any Jira update (status, fields, links, etc.) without explicit user approval. When drafting Jira text: ASCII hyphens only, copy-pastable, Jira wiki markup so it renders in Jira (not GitHub markdown), hyperlink ticket/comment IDs, readable by product/QA (not only engineers), and self-contained so the reader has no leftover questions. Scenario or confirmation lists include S.No.; ask objectors to cite seq no, what is wrong in the current approach, and the expected behavior.
